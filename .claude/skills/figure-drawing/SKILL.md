...
`lint_figures.py` takes **over five minutes** across the thirteen families — background it or ask
...
**Rebuild all thirteen families and check `git status`.** Every existing figure must come back
...
for f in eip_figures coupling_grids if_later queues_streams integration_styles \
         ...
         asyncapi_figures rmq_figures; do
