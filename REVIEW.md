...
| **R0-5** | **A fourth arrangement: text left, drawing right.** Not asked for by name, but **eight of Ian's rows need it** — #56/57, #58/59, #60/61, #62, #64/65, #68/69 — all the same instruction: *"shrink the diagram… put the text on the left"*. The builder has three arrangements today and the figure-leads one exists **because `styles.md`'s panel costs a drawing 47–63% of its label size**. Ian is overriding that for these slides knowingly. **Report what each one actually costs when the row is done**, per figure, and let him keep or revert it | M | ☐ |
